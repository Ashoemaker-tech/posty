      ...
      @can('delete', $post)
        ...
        <form action="{{ route('posts.destroy', $post) }}" method="post">
            ...
            <div class="flex justify-end ">
                <button class="btn btn-error bg-red-500 border-red-500 btn-circle btn-xs text-base-100">
    ...
    <div class="flex items-center">
        ...
        @endauth
            <span class="mx-1">{{ $post->likes->count() }} {{ Str::plural('like', $post->likes->count()) }}</span>
                <a href="{{ route('comment', $post) }}"  class="text-white ml-2 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<!-- Delete Post Modal -->
<input type="checkbox" id="delete-post-modal" class="modal-toggle" />
<label for="delete-post-modal" class="modal cursor-pointer">
<label class="modal-box relative" for="">
<label for="delete-post-modal" class="btn btn-error btn-xs bg-red-500 text-white border-red-500 btn-circle absolute right-4 top-4">✕</label>
